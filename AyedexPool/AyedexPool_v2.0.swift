...
func difficultyToTarget(_ difficulty: Double) -> Data {
    ...
    guard let maxTarget = Data(hexString: maxTargetHex) else {
...
/// Convert compact "bits" to a 32-byte target for hash comparison.
/// A hash that is lexicographically <= this target meets the network difficulty.
func bitsToTarget(_ bits: String) -> Data {
    guard let bitsVal = UInt32(bits, radix: 16) else { return Data(repeating: 0, count: 32) }
    let exp = Int((bitsVal >> 24) & 0xff)
    let mant = bitsVal & 0x007fffff
    var target = [UInt8](repeating: 0, count: 32)
    let start = 32 - exp  // byte position of the most significant mantissa byte
    if start >= 0 && start < 32     { target[start]     = UInt8((mant >> 16) & 0xff) }
    if start + 1 >= 0 && start + 1 < 32 { target[start + 1] = UInt8((mant >> 8)  & 0xff) }
    if start + 2 >= 0 && start + 2 < 32 { target[start + 2] = UInt8( mant        & 0xff) }
    return Data(target)
}

/// Compare two 32-byte hashes: returns true if a <= b (little-endian block hash comparison).
func hashMeetsTarget(_ hashReversed: Data, target: Data) -> Bool {
    // Both are big-endian (display order) 32-byte values
    for i in 0..<32 {
...
/// Compute share difficulty from a 32-byte hash (big-endian / display order).
func hashToDifficulty(_ hashReversed: Data) -> Double {
    // difficulty = maxTarget / hashTarget
    // Quick approximation: count leading zero bits + fractional from first nonzero bytes
    var value: Double = 0
    var foundNonZero = false
    for (i, byte) in hashReversed.enumerated() {
        if byte != 0 || foundNonZero {
            if !foundNonZero { foundNonZero = true }
            value = value * 256.0 + Double(byte)
            if i > 4 { break } // enough precision
        }
    }
    if value == 0 { return Double.infinity }
    let zeroBits = countLeadingZeroBits(hashReversed)
    // maxTarget has 32 leading zero bits at diff 1
    // shareDiff ≈ 2^(zeroBits - 32) * correction
    let shift = Double(zeroBits) - 32.0
    return pow(2.0, shift) * (256.0 / Double(hashReversed[zeroBits / 8]))
}
...
class BlockTemplateManager {
    ...
    func addressToScript(_ address: String) -> String {
        ...
        } else if address.hasPrefix("nx1q") {
            // NEX P2WPKH (witness v0, 20-byte program)
            if let decoded = bech32Decode(address) {
                return "0014" + decoded
            }
        } else if address.hasPrefix("nex1z") {
            // NEX PQ witness v2 (32-byte program)
            if let decoded = bech32Decode(address) {
                return "5220" + decoded
            }
        } else if address.hasPrefix("1") || address.hasPrefix("N") {
            // P2PKH (legacy)
        ...
        // Fallback: try bech32 decode for any unknown prefix
        if address.contains("1"), let decoded = bech32Decode(address) {
            // Determine witness version from first data byte
            let charset = "qpzry9x8gf2tvdw0s3jn54khce6mua7l"
            if let sepIdx = address.lastIndex(of: "1") {
                let firstDataChar = address[address.index(after: sepIdx)]
                if let idx = charset.firstIndex(of: firstDataChar) {
                    let witVer = charset.distance(from: charset.startIndex, to: idx)
                    return String(format: "%02x", 0x50 + witVer) + String(format: "%02x", decoded.count / 2) + decoded
                }
            }
        }
        print("[WARNING] Could not decode address \(address), using OP_RETURN!")
        return "6a"
    }
...
class StratumServer {
    ...
    func handleSubmit(client: StratumClient, id: Any?, params: [Any]) {
        ...
        // ── Validate the share by computing the actual block header hash ──
        let validation = validateShare(client: client, jobId: jobId, extranonce2: extranonce2, ntime: ntime, nonce: nonce)
        let zeros = validation.zeros
        let blockHash = validation.hash

        guard validation.valid, !blockHash.isEmpty, let hashData = Data(hexString: blockHash) else {
            client.sharesRejected += 1
            PoolStats.shared.totalSharesRejected += 1
            client.sendResult(id: id ?? 0, result: nil, error: [20, "Invalid share", nil])
            let ts = logTimestamp()
            print("\(orange)₿\(reset) (\(gold)\(ts)\(reset)) \(cyan)stratum_task:\(reset) \(red)share invalid (bad hash)\(reset)")
            return
        }

        // Compute actual share difficulty from the hash
        let actualDiff = hashToDifficulty(hashData)
        let ts = logTimestamp()
        print("\(orange)₿\(reset) (\(gold)\(ts)\(reset)) \(cyan)asic_result:\(reset) ID: \(purple)\(jobId)\(reset), ver: \(pink)\(versionBits)\(reset) Nonce \(yellow)\(nonce.uppercased())\(reset) diff \(purple)\(String(format: "%.1f", actualDiff))\(reset) of \(Int(client.difficulty)). hash: \(yellow)\(blockHash.prefix(16))...\(reset)")

        // Check if meets share difficulty
        let accepted = actualDiff >= client.difficulty
        guard accepted else {
            client.sharesRejected += 1
            ...
            print("\(orange)₿\(reset) (\(gold)\(ts2)\(reset)) \(cyan)stratum_task:\(reset) \(red)share rejected (diff \(String(format: "%.1f", actualDiff)) < \(Int(client.difficulty)))\(reset)")
            return
        }
        ...
        if actualDiff > client.bestDifficulty { client.bestDifficulty = actualDiff }
        ...
        print("\(orange)₿\(reset) (\(gold)\(ts2)\(reset)) \(cyan)stratum_task:\(reset) \(green)share accepted\(reset) diff=\(String(format: "%.1f", actualDiff)) zeros=\(zeros)")

        // ── Check if share meets NETWORK target (i.e., we found a block!) ──
        guard let template = templateManager.currentTemplate,
              let bits = template["bits"] as? String else { return }

        let networkTarget = bitsToTarget(bits)
        if hashMeetsTarget(hashData, target: networkTarget) {
            let height = (template["height"] as? Int) ?? PoolStats.shared.currentBlockHeight
            let reward = Double((template["coinbasevalue"] as? Int) ?? 10_000_000_000) / 100_000_000.0

            print("")
            print("\(orange)₿₿₿\(reset) \(green)\u{001B}[1m⛏️  BLOCK FOUND!  ⛏️\u{001B}[0m\(reset) \(orange)₿₿₿\(reset)")
            print("\(orange)₿\(reset) (\(gold)\(ts2)\(reset)) \(cyan)stratum_task:\(reset) \(green)\u{001B}[1mBLOCK FOUND at height \(height)!\u{001B}[0m\(reset)")
            print("\(orange)₿\(reset) (\(gold)\(ts2)\(reset)) \(cyan)stratum_task:\(reset) Hash: \(yellow)\(blockHash)\(reset)")
            print("\(orange)₿\(reset) (\(gold)\(ts2)\(reset)) \(cyan)stratum_task:\(reset) Miner: \(purple)\(worker)\(reset) Reward: \(green)\(String(format: "%.2f", reward)) NEX\(reset)")
            print("")

            // Assemble the full block and submit to the node
            assembleAndSubmitBlock(client: client, extranonce2: extranonce2, ntime: ntime, nonce: nonce)

            // Notify the miner that found the block
            client.sendNotify(method: "mining.block_found", params: [blockHash, height, reward])

            // Notify ALL connected miners
            clientLock.lock()
            let allClients = Array(clients.values)
            clientLock.unlock()
            for c in allClients where c.id != client.id && c.subscribed {
                c.sendNotify(method: "mining.block_found", params: [blockHash, height, reward])
            }

            PoolStats.shared.blocksFound += 1

            // Refresh template for new block
            if templateManager.refreshTemplate() {
                broadcastWork()
            }
        }
    }

    /// Assemble the full serialized block from template + miner's solution and submit via RPC.
    func assembleAndSubmitBlock(client: StratumClient, extranonce2: String, ntime: String, nonce: String) {
        guard let template = templateManager.currentTemplate else { return }
        ...
        guard let coinbase = templateManager.buildCoinbaseTransaction(
            ...
            print("\(orange)₿\(reset) \(red)Failed to build coinbase for block submission\(reset)")
            return
        }
        ...
        // Assemble full block: header + varint(txcount) + coinbase + transactions
        var blockHex = header

        // Transaction count (coinbase + mempool txs)
        var txData: [String] = []
        if let transactions = template["transactions"] as? [[String: Any]] {
            txData = transactions.compactMap { $0["data"] as? String }
        }
        let txCount = 1 + txData.count
        if txCount < 0xfd {
            blockHex += String(format: "%02x", txCount)
        } else {
            blockHex += String(format: "fd%04x", txCount)
        }

        // Coinbase transaction
        blockHex += coinbase.txHex
        ...
        // Submit to node
        let ts = logTimestamp()
        print("\(orange)₿\(reset) (\(gold)\(ts)\(reset)) \(cyan)stratum_task:\(reset) Submitting block (\(blockHex.count / 2) bytes) to node...")

        let submitResult = templateManager.rpc.submitBlock(hexData: blockHex)
        if submitResult == nil {
            // nil result means success for submitblock
            print("\(orange)₿\(reset) (\(gold)\(ts)\(reset)) \(cyan)stratum_task:\(reset) \(green)\u{001B}[1m✓ Block accepted by node!\u{001B}[0m\(reset)")
        } else {
            print("\(orange)₿\(reset) (\(gold)\(ts)\(reset)) \(cyan)stratum_task:\(reset) \(red)Block rejected: \(submitResult ?? "unknown error")\(reset)")
        }
    }
